/**
 * @file GroupLData_Serialized.hpp
...
/**
 * A class for space-efficient data type of grouped L-node.
 *
...
template <size_type t_GroupBound, bool t_SumExpHint = true, bool t_LookupHint = true,
          size_type t_GrowSize = t_GroupBound / 8>
class GroupLData_Serialized {
    static_assert(sizeof(uchar_type) == 1);

  public:
    using this_type  = GroupLData_Serialized<t_GroupBound, t_SumExpHint, t_LookupHint, t_GrowSize>;
    using fnode_type = typename LinkedList<GroupFData_Serialized<this_type>>::node_type;
    ...
    class lcursor_type {
        friend class GroupLData_Serialized;
  ...
  private:
    ...
    offset_type m_hofst  = 0;  // Compressible, but would not be reasonable
    uint8_t     m_weight = 0;
  ...
  public:
    GroupLData_Serialized() = default;

    //! Default destructor
    virtual ~GroupLData_Serialized() = default;

    //! Copy constructor (deleted)
    GroupLData_Serialized(const GroupLData_Serialized&) = delete;

    //! Copy constructor (deleted)
    GroupLData_Serialized& operator=(const GroupLData_Serialized&) = delete;

    //! Move constructor
    GroupLData_Serialized(GroupLData_Serialized&&) noexcept = default;

    //! Move constructor
    GroupLData_Serialized& operator=(GroupLData_Serialized&&) noexcept = default;
