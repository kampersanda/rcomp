/**
 * @file GroupedFData_Serialized.hpp
 */
#pragma once
...
class GroupedFData_Serialized {
  public:
    using this_type = GroupedFData_Serialized<t_LData>;
    ...
    // Cursor
    class fcursor_type {
        friend class GroupedFData_Serialized;
  ...
  public:
    //! Default constructor
    GroupedFData_Serialized() = default;

    //! Default destructor
    virtual ~GroupedFData_Serialized() = default;

    //! Copy constructor (deleted)
    GroupedFData_Serialized(const GroupedFData_Serialized&) = delete;

    //! Copy constructor (deleted)
    GroupedFData_Serialized& operator=(const GroupedFData_Serialized&) = delete;

    //! Move constructor
    GroupedFData_Serialized(GroupedFData_Serialized&&) noexcept = default;

    //! Move constructor
    GroupedFData_Serialized& operator=(GroupedFData_Serialized&&) noexcept = default;
    ...
    // Search the most frontward run-unit whose order <= q_order and second_order <= q_second_order.
    inline std::optional<fcursor_type> successor(const loint_type q_order, const size_type q_second_order) const {
        size_type frpos = 0;
        for (; frpos < get_num_runs(); frpos++) {
            const lnode_type* lnode = get_lnode_from_position(frpos);
            const size_type lrnid = get_lrnid_from_position(frpos);
        ...
        if (frpos == get_num_runs()) {
            // return {MAX_SIZE_INT, MAX_SIZE_INT};
            return std::nullopt;
        }
        return fcursor_type{get_frnid_from_position(frpos), frpos};
    }
