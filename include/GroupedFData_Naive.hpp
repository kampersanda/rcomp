...
class GroupedFData_Naive {
  public:
    using this_type = GroupedFData_Naive<t_LData>;
    ...
    // Cursor
    class fcursor_type {
        friend class GroupedFData_Naive;
      ...
      public:
        ...
        inline size_type get_frnid() const {
            return m_frnid;
        }
        inline size_type get_frpos() const {
            return m_frpos;
        }

        inline bool operator==(const fcursor_type& other) const {
            return m_frnid == other.m_frnid;
        }
        inline bool operator!=(const fcursor_type& other) const {
            return m_frnid != other.m_frnid;
        }
  ...
  public:
    //! Default constructor
    GroupedFData_Naive() = default;

    //! Default destructor
    virtual ~GroupedFData_Naive() = default;

    //! Copy constructor (deleted)
    GroupedFData_Naive(const GroupedFData_Naive&) = delete;

    //! Copy constructor (deleted)
    GroupedFData_Naive& operator=(const GroupedFData_Naive&) = delete;

    //! Move constructor
    GroupedFData_Naive(GroupedFData_Naive&&) noexcept = default;

    //! Move constructor
    GroupedFData_Naive& operator=(GroupedFData_Naive&&) noexcept = default;
    ...
    // Search the most backword run-unit whose (order, second_order) <= (q_order, q_second_order).
    ...
    // Search the most frontward run-unit whose (q_order, q_second_order) <= (order, second_order).
    std::optional<fcursor_type> successor(const loint_type q_order, const size_type q_second_order) const {
        size_type i = 0;
        for (; i < m_units.size(); i++) {
            const unit_type& unit = m_units[i];
            const loint_type order = lo_common::get_basic_order(unit.lnode);
        ...
        return fcursor_type{m_units[i].frnid, i};
    }
    ...
    fcursor_type split_after(fcursor_type& fcrsr, lnode_type* new_lnode, size_type new_lrnid) {
        ...
        // NOTE: m_sum_exps is not incremented because of split only.
