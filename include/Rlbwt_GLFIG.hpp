...
/**
 * A RLBWT class with the grouped LF-interval graph.
 *
 * @tparam t_Graph The grouped LF-interval graph.
 */
template <class t_Graph>
class Rlbwt_GLFIG : public Rlbwt_Interface {
  public:
    using this_type = Rlbwt_GLFIG<t_Graph>;
  ...
  public:
    ...
    //! Default destructor
    virtual ~Rlbwt_GLFIG() = default;

    //! Copy constructor (deleted)
    Rlbwt_GLFIG(const Rlbwt_GLFIG&) = delete;

    //! Copy constructor (deleted)
    Rlbwt_GLFIG& operator=(const Rlbwt_GLFIG&) = delete;

    //! Move constructor
    Rlbwt_GLFIG(Rlbwt_GLFIG&&) noexcept = default;

    //! Move constructor
    Rlbwt_GLFIG& operator=(Rlbwt_GLFIG&&) noexcept = default;
    ...
    void extend(const uchar_type new_chr) override {
        ...
        // Phase A
        STAT_MONITOR(m_timer_4_case.start();)
        const auto ins_mode = m_graph.check_mergable(new_chr);
