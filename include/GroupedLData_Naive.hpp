...
/**
 * A grouped L-node data type in a naive manner.
 *
 * @tparam t_GroupedBound The upper bound of storable runs.
 ...
 */
template <size_type t_GroupedBound, bool t_WithSAE, bool t_SumExpHint = true, bool t_LookupHint = true>
class GroupedLData_Naive {
  public:
    using this_type = GroupedLData_Naive<t_GroupedBound, t_WithSAE, t_SumExpHint, t_LookupHint>;
    using fnode_type = typename LinkedList<GroupedFData_Naive<this_type>>::node_type;
    ...
    // Cursor
    class lcursor_type {
        friend class GroupedLData_Naive;
  ...
  public:
    //! Default constructor
    GroupedLData_Naive() = default;

    //! Default destructor
    virtual ~GroupedLData_Naive() = default;

    //! Copy constructor (deleted)
    GroupedLData_Naive(const GroupedLData_Naive&) = delete;

    //! Copy constructor (deleted)
    GroupedLData_Naive& operator=(const GroupedLData_Naive&) = delete;

    //! Move constructor
    GroupedLData_Naive(GroupedLData_Naive&&) noexcept = default;

    //! Move constructor
    GroupedLData_Naive& operator=(GroupedLData_Naive&&) noexcept = default;
    ...
    size_type get_memory_in_bytes(bool include_this = true) const {
        ...
        return this_bytes + sizeof(unit_type) * m_units.capacity();
    }
    ...
    size_type get_sae(size_type lrnid) const {
    ...
    size_type get_sae(lcursor_type lcrsr) const {
        if constexpr (WITH_SAE) {
            DEBUG_ABORT_IF(!is_valid_lcursor(lcrsr));
            return m_units[lcrsr.get_lrpos()].sae;
        } else {
            ABORT_IF(true);
        }
    }
    void set_sae(size_type lrnid, size_type sae) {
        if constexpr (WITH_SAE) {
    ...
    void set_sae(lcursor_type lcrsr, size_type sae) {
        if constexpr (WITH_SAE) {
            DEBUG_ABORT_IF(!is_valid_lcursor(lcrsr));
            m_units[lcrsr.get_lrpos()].sae = sae;
        } else {
            ABORT_IF(true);
        }
    }
    ...
    // Return the run-ID
    lcursor_type insert_after(const lcursor_type& lcrsr, const uchar_type new_chr, const size_type new_exp) {
        DEBUG_ABORT_IF(!is_valid_lcursor(lcrsr));
